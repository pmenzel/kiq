...
void usage_kdump() {
	print_usage_header();
	fprintf(stderr, "Usage:\n   kiq dump -i <file> -k <file> -m <file> -p <mode>\n");
	fprintf(stderr, "\n");
...
int main_kdump(int argc, char** argv) {
	...
	int c;
	while ((c = getopt(argc, argv, "hdvi:k:m:p:")) != -1) {
		switch (c)  {
			case 'h':
				usage_kdump();
			case 'd':
	...
	if(filename_meta.length() == 0) { error("Please specify the name of the metadata file, using the -m option."); usage_kdump(); }
	...
	if(mode=="db") {
		...
		for(auto const & it : exp_id2name) {
			...
			ReadCount readcount = exp_id2readcount.at(it.first);
			std::cout << exp_id << "\t" << exp_name << "\t" << readcount << "\n";
		}
	}
	else if(mode=="stats") {
